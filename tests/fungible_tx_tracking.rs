...
use lmscan_agent::{service::api_service::ApiService, transaction::{TransactionWithResult, Common, RewardTx, Transaction, TokenTx, TxJob}, tx_state};
use dotenvy::{var};
...
async fn fungible_tx_tracking() {
  ...
  for (hash, tx_res) in tx_results.clone() {
    ...
    let len = inputs.len();
    output_file.write(format!("INPUT TX LIST [{len}]\n").as_bytes()).unwrap();
    for input_hash in inputs {
      let input_tx = serde_json::to_string_pretty(&ApiService::get_tx_always(&input_hash).await).unwrap();
      output_file.write(format!("{input_hash}\n{input_tx}\n").as_bytes()).unwrap();
    }
