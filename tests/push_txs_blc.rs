...
async fn push_txs_blc() {
  ...
  let blocks: Vec<(String, i64)> = block_entity::Entity::find()
                                    .select_only()
                                    .columns([
                                      block_entity::Column::Hash, 
                                      block_entity::Column::Number,
                                    ])
  ...
  let block_hashs: Vec<String> = blocks.into_iter()
  ...
  let block_txs_map: HashMap<String, Vec<String>> = tx_entity::Entity::find()
      .select_only()
      .columns([
        tx_entity::Column::BlockHash, 
        tx_entity::Column::Json,
      ])
  ...
  for (i, block_hash) in block_hashs.into_iter().enumerate() {
    ...
    if let Some(txs) = block_txs_map.get(&block_hash) {
      let tx_res_vec: Vec<TransactionWithResult> = txs.into_iter().map(|tx| TransactionWithResult::from(tx).unwrap()).collect();
      let txs: Vec<String> = tx_res_vec.iter().map(|tx_res| serde_json::to_string(&tx_res.signed_tx).unwrap()).collect();
      let body_str = format!("[{}]", txs.join(","));
      println!("body: {body_str}");
      match ApiService::post_txs(body_str).await {
        Ok(result_hashs) => if result_hashs.is_empty() {
          panic!("post 실패");
        },
