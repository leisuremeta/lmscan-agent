...
use lmscan_agent::{service::api_service::ApiService, balance_entity, account_entity};
...
async fn free() {
  ...
  let scan_accounts: HashSet<String> = account_entity::Entity::find().all(db).await.unwrap().into_iter().map(|a|a.address).collect();
  let balances = balance_entity::Entity::find().all(db).await.unwrap();
  let balances: Vec<(String, BigDecimal)> = balances.into_iter().filter(|b| scan_accounts.contains(&b.address)).map(|b| (b.address, b.free)).collect();
    ...
    if let Some(block_account_balance_opt) = ApiService::get_free_balance(&address).await.ok() {
      ...
      if let Some(block_info) = block_account_balance.get("LM") {
        let line = format!("{}, {}, {}, {}, {}\n", 
                  ...
                  if block_info.total_amount.clone() > free { block_info.total_amount.clone() - free } else { free - block_info.total_amount.clone() }
                );
    ...
    if let Some(block_account_balance_opt) = ApiService::get_locked_balance(&address).await.ok() {
      ...
      if let Some(block_info) = block_account_balance.get("LM") {
        let line = format!("{}, {}, {}, {}, {}\n", 
                  ...
                  if block_info.total_amount.clone() > locked { block_info.total_amount.clone() - locked } else { locked - block_info.total_amount.clone() }
                );
        output_file.write(line.as_bytes()).expect("write failed");
      }
    }
