...
use crate::library::common::{into_byte_vec, from_ivec};
...
impl<K: Serialize + for<'a> Deserialize<'a> + Clone, V: Serialize + for<'a> Deserialize<'a> + Default> TypedSled<K, V> {
  ...
  pub fn entry(&self, key: &K) -> Option<(K, V)> {
    Self::get(&self, key)
         .map(|deserialized_val| (key.clone(), deserialized_val))
  }
