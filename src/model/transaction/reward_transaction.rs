...
use crate::{library::common::{as_timestamp, now, from_rawvalue_to_bigdecimal_map, as_vec}, tx_entity::ActiveModel};

use super::{common::Common, TransactionWithResult, TransactionResult};
...
impl Common for RecordActivity {
  ...
  fn from(&self, hash: String, from_account: String, block_hash: String, block_number: i64, json: String, _: TransactionWithResult) -> ActiveModel {
    ActiveModel {
      hash: Set(hash),
...
impl Common for ExecuteReward {
  fn created_at(&self) -> i64 { as_timestamp(self.created_at.as_str()) }
  fn network_id(&self) -> i64 { self.network_id }
  fn from(&self, hash: String, from_account: String, block_hash: String, block_number: i64, json: String, tx: TransactionWithResult) -> ActiveModel {
