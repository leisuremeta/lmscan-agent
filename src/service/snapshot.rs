...
use log::info;
use crate::entity::{balance_entity, block_state, state_daily, block_daily};
...
pub async fn daily_snapshot(db: &DatabaseConnection) {
  ...
  let start_of_day = (now - Duration::days(10)).date_naive().and_hms_opt(15, 0, 0).unwrap().timestamp();
  ...
  let outer_vec: Vec<Vec<state_daily::ActiveModel>> = daily_states.into_iter()
                                                    .chunks(100)
                                                    .into_iter()
                                                    .map(|m| m.collect())
                                                    .collect();
  ...
  for daily_states in outer_vec {
    if let Err(err) = state_daily::Entity
                                        ::insert_many(daily_states)
                                        .exec(&txn).await {
  ...
  Fut: Future<Output = Result<Option<HashMap<String, BalanceInfo>>, String>>,
{
  ...
  let total_amount = response.get("LM")
